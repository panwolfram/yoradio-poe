...
void NetServer::onWsMessage(void *arg, uint8_t *data, size_t len, uint32_t clientId) {
    ...
    if (config.parseWsCommand((const char*)data, cmd, val, 65)) {
      ...
      if (strcmp(cmd, "rebootmdns") == 0) {
        ...
        if(strlen(config.store.mdnsname)>0)
          ...
          snprintf(buf, MDNS_LENGTH*2, "{\"redirect\": \"http://%s/\"}", ETH.localIP().toString().c_str());
  ...
  // (Optional) If you later expect large/fragmented frames, add an
  // accumulator keyed by clientId and feed it until info->final==true.
}

void NetServer::getPlaylist(uint32_t clientId) {
  char buf[160] = {0};
  sprintf(buf, "{\"file\": \"http://%s%s\"}", ETH.localIP().toString().c_str(), PLAYLIST_PATH);
