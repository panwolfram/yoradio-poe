#line 1 "C:\\Users\\mwolfram\\Documents\\Arduino Projects\\yoradio\\yoRadio\\src\\core\\telnet.cpp"
#include <stdarg.h>
#include "WiFi.h"
...
bool Telnet::begin(bool quiet) {
  ...
  if (ETH.linkUp() == true) {
    ...
    if(!quiet){
      Serial.println("done");
      Serial.println("##[BOOT]#");
      BOOTLOG("Ready! Go to http:/%s/ to configure", ETH.localIP().toString().c_str());
      BOOTLOG("------------------------------------------------");
      Serial.println("##[BOOT]#");
    }
    return true;
...
void Telnet::on_input(const char* str, uint32_t clientId) {
